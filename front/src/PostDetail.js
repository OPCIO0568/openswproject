...
function PostDetail() {
  ...
  const fetchPostDetail = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/public/donations/detail", {
        method: "POST",
        headers: {
      ...
      setPost(data.data); // Update post data with the latest info
      setLoading(false);
    } catch (err) {
  ...
  // Fetching post details and username on component mount
  useEffect(() => {
    fetchPostDetail();
  ...
  const handleDonation = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/api/private/donations/donate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="post-detail-container">
        <div className="post-detail-upper-box">
          ...
          <div className="post-detail-image-container">
            <img
              src={post.mainImage} // Display the post image using post.mainImage
              alt={post.title}
